...
> **📖 어떤 하네스를 써야 할지 모르겠다면 → [하네스 상세 카탈로그](harness-catalog.html)**
> 103종 전체를 카테고리 탭으로 정리했습니다. 하네스마다 **무엇을 하는가 · 언제 쓰는가 · 산출물 · 에이전트 구성**을 담았고, 키워드 검색으로 전 카테고리를 한 번에 찾을 수 있습니다. (브라우저로 파일을 열면 됩니다)
